...
public class ClassRingAdapter extends BaseExpandableListAdapter implements
	...
	int picHeight;
	public List<ClassRingModel> classRingList;// 班级圈列表
	public List<ClassRingModel> replyList;// 回复列表
	LinearLayout layout;
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		groupHolder.comment.setTag(R.id.bundle_params1, view.getHeight());
	...
	public void onClick(final View v) {
		...
		case R.id.delete:// 删除
			...
			IConstant.HTTP_CONNECT_POOL.addRequest(
					IUrContant.DELETE_CLASS_RING_URL, map, adapetHandler,
					params, true);
			break;
		case R.id.zan:// 赞
			...
			IConstant.HTTP_CONNECT_POOL.addRequest(
					IUrContant.PRASE_CLASS_RING_URL, map, adapetHandler,
					params, true);
			break;
		case R.id.zhedie:
	...
	public void httpSuccess(HttpResponseModel message) throws Exception {
		if (message.getUrl().equals(IUrContant.DELETE_CLASS_RING_URL)) {// 删除成功
			...
			if (data.optInt("retcode") == 1) {
				...
				classRingList.remove(position);
				notifyDataSetChanged();
			}
		} else if (message.getUrl().equals(IUrContant.PRASE_CLASS_RING_URL)) {// 赞成功
			JSONObject data = new JSONObject(new String(message.getResponse()));
			if (data.optInt("retcode") == 1) {
				Map<String, Object> params = message.getMap();
				int position = (int) params.get("position");
				String string = classRingList.get(position).getPriase();
				if (string != null) {
					string = string + "、";
				} else {
					string = "";
